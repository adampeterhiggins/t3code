  ...
  it.live("scopes catalog rates to Devin when provider model names collide", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.isFalse(withoutDevin.pricing.source.includes("Devin provider snapshot pricing"));
  ...
  it.live("keeps Devin catalog rates when LiteLLM loads on the first scan", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.include(first.pricing.source, "Devin provider snapshot pricing");

        yield* TestClock.adjust(Duration.minutes(2));
        const refreshed = yield* service.refreshRates;
        assert.strictEqual(refreshed.knownModels, 2);
        assert.include(refreshed.source, "Devin provider snapshot pricing");
        ...
        assert.isFalse(
          withoutDevinCatalog.pricing.source.includes("Devin provider snapshot pricing"),
        );
      }).pipe(
        Effect.provide(
          serviceLayers({
